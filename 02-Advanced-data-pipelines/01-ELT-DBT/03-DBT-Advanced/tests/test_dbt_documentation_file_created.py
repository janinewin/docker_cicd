...
all_sub_dir_short = [(re.sub(r'^.*?dbt_lewagon', 'dbt_lewagon', root),
                       dirs,
                       files) for (root, dirs, files) in all_sub_dir]

def test_documentation_file_created():
    mart_structure = [x for x in all_sub_dir_short if x[0] == 'dbt_lewagon/models/mart']

    assert len(mart_structure) == 1, "dbt_lewagon/models/mart directory has not been created"
    if len(mart_structure) == 1:
        assert 'models.yml' in mart_structure[0][2], "models.yml not created in dbt_lewagon/models/mart directory"
