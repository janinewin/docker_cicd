...
class TestOptionalBigQueryDag:
    ...
    def test_dag_config(self):
        ...
        dag = self.dagbag.get_dag(dag_id="optional_google_cloud")
    ...
    def test_load_to_bigquery_task(self):
        ...
        for month in range(6, 7):
            ...
            assert ti.task.skip_leading_rows == 1
            assert ti.task.schema_fields == [
                {"name": "month", "type": "STRING"},
                {"name": "trip_distance", "type": "FLOAT"},
                {"name": "total_amount", "type": "FLOAT"},
            ]
