...
def test_load_manifest():
    data = dbt.load_manifest('tests/dbt_light/target/manifest.json')
    assert list(data.keys()) == ['metadata', 'nodes', 'sources', 'macros', 'docs', 'exposures', 'selectors', 'disabled', 'parent_map', 'child_map']
    assert list(data['nodes']) == [
        'model.dbt_lewagon.my_first_dbt_model', 'test.dbt_lewagon.unique_my_first_dbt_model_id.16e066b321',
    ...
    data = dbt.load_manifest('tests/dbt/target/manifest.json')
    assert list(data.keys()) == ['metadata', 'nodes', 'sources', 'macros', 'docs', 'exposures', 'selectors', 'disabled', 'parent_map', 'child_map']
    assert list(data['nodes']) == [
        'model.dbt_lewagon.my_first_dbt_model', 'model.dbt_lewagon.my_second_dbt_model',
        'test.dbt_lewagon.unique_my_first_dbt_model_id.16e066b321',
        'test.dbt_lewagon.not_null_my_first_dbt_model_id.5fb22c2710',
...
def test_create_tasks():
    ...
    bash_command_by_task_name = {
        ...
        'test.dbt_lewagon.unique_my_first_dbt_model_id.16e066b321': f'{dbt_no_write} test {target} --models unique_my_first_dbt_model_id {dir_locations}',
        'test.dbt_lewagon.not_null_my_first_dbt_model_id.5fb22c2710': f'{dbt_no_write} test {target} --models not_null_my_first_dbt_model_id {dir_locations}',
    }

    tasks = dbt.create_tasks(data)
    ...
    bash_command_by_task_name = {
        ...
        'test.dbt_lewagon.unique_my_first_dbt_model_id.16e066b321': f'{dbt_no_write} test {target} --models unique_my_first_dbt_model_id {dir_locations}',
        'test.dbt_lewagon.not_null_my_first_dbt_model_id.5fb22c2710': f'{dbt_no_write} test {target} --models not_null_my_first_dbt_model_id {dir_locations}',
        'test.dbt_lewagon.unique_my_second_dbt_model_id.57a0f8c493': f'{dbt_no_write} test {target} --models unique_my_second_dbt_model_id {dir_locations}',
        'test.dbt_lewagon.not_null_my_second_dbt_model_id.151b76d778': f'{dbt_no_write} test {target} --models not_null_my_second_dbt_model_id {dir_locations}',
    }
...
def test_create_dags_dependencies():
    ...
    with DAG(
        ...
        data = dbt.load_manifest('tests/dbt_light/target/manifest.json')
        dbt.create_dags_dependencies(data, dbt.create_tasks(data))
