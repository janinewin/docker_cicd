...
def test_dbt_profile_location():
    assert os.path.exists(dbt_full_profile_path), "Your profiles.yml file is not located in the correct folder. It should be under ~/.dbt/"
...
def test_dbt_correct_keys():
    assert 'outputs' in profile_dict['dbt_lewagon'], "outputs should be below dbt_lewagon"
    assert 'dev' in  profile_dict['dbt_lewagon']['outputs'], "dev should be one of your defined targets"
    assert 'dataset' in  profile_dict['dbt_lewagon']['outputs']['dev'], "within the dev target, the key 'dataset' should be defined"
