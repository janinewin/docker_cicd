...
def test_mart_user_structure_first_exercice():
    ...
    if len(df) >= 1:
        ...
        assert expected_columns.issubset(actual_columns), "mart_user model : there are some missing columns"
...
def test_mart_user_structure_second_exercice():
    ...
    if len(df) >= 1:
        actual_columns = set(df['column_name'].tolist())
        assert 'user_url'in actual_columns, "mart_user model : the user_url field has not been added"
