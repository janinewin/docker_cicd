...
class TestTasksConfigs:
    ...
    def test_tasks(self):
        ...
        assert list(map(lambda task: task.task_id, dag.tasks)) == ["create_file_if_not_exist", "get_quote_and_save_if_new"]
    ...
    def test_create_file_if_not_exist_task(self):
        ...
        assert list(map(lambda task: task.task_id, task.downstream_list)) == ["get_quote_and_save_if_new"]
    ...
    def test_get_quote_and_save_if_new_task(self):
        ...
        assert task.op_kwargs == {"quotes_file": "/opt/airflow/data/quotes.csv"}
        assert list(map(lambda task: task.task_id, task.upstream_list)) == ["create_file_if_not_exist"]
