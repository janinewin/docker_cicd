...
def test_table_creation():
    prep_db()
    df = lewagonde.read_sql_query("SELECT * FROM ikea_products LIMIT 1;", password=os.environ["POSTGRES_PASSWORD"], user="lewagon", host="0.0.0.0", port="5433", dbname="ikea")
    columns = set([
        "product_title", "product_url", "sku", "mpn", "currency", "product_price",
        "product_condition", "availability", "seller", "seller_url", "brand", "raw_product_details",
        "breadcrumbs", "country", "language", "average_rating", "reviews_count",
...
def test_data_loaded():
    ...
    """
    df = lewagonde.read_sql_query("SELECT COUNT(*) AS count FROM ikea_products;", password=os.environ["POSTGRES_PASSWORD"], user="lewagon", host="0.0.0.0", port="5433", dbname="ikea")
...
def test_btree_index():
    ...
    """
    _, results = query_perf(sql_query="select count(*) as count from pg_indexes where indexname = 'index_sku';")
...
def text_fulltext():
    ...
    """
    _, results = query_perf(sql_query="select count(*) as count from pg_indexes where indexname = 'index_raw_product_details_text';")
...
def test_search_chair_indexed():
    ...
    _, results = query_perf(sql_file_path=sql_file_path)
    assert results.iloc[0, 0] > 25, "search-chair-indexed.sql seems wrong, finding less than 25 values, expected more"
