...
The goal of this exercise is to continue the work on Wednesday's exercises: **Twitter API**.
...
If you get the following error: `sqlalchemy.exc.OperationalError`, please check your `DATABASE_URL`. Your password should not countain the symbols: `<`, `>`.
...
It must look like `2022.x`. If not, please ask a TA.
...
We have seen in the lecture that `docker-compose` was used to define multiple services, and bring up the application stack with the `docker-compose up` command.
...
:point_right: Bring up the stack, running containers in the background, and re-building the image for `web` : ```docker compose up -d``` 🛠
...
:point_right: Eventually, run your migrations: ```docker-compose run web flask db init ```
...
* `web` depends on `db` to be up and healthy.
