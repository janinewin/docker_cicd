...
def test_docker_compose():
    ...
    with open(dc_fp) as f:
        dc = yaml.safe_load(f)
    neo4j_services = [service for service in dc["services"].values() if service["image"].startswith("neo4j:")]
...
def test_dgraph():
    ...
    with open(dc_fp) as f:
        dc = yaml.safe_load(f)
    dgraph_services = [service for service in dc["services"].values() if service["image"].startswith("dgraph")]
    assert len(dgraph_services) == 3, "DGraph services not found in the docker-compose.yml"
